...
WAN_IP = "yourpublicip"
LOCAL_IP_PREFIX_1 = "10.168."
LOCAL_IP_PREFIX_2 = "172.168."
LOCAL_IP_PREFIX_3 = "192.168."
LOCAL_IP_PREFIX_4 = "192.168."
WHITELIST_IPS = (WAN_IP, LOCAL_IP_PREFIX_1, LOCAL_IP_PREFIX_2, LOCAL_IP_PREFIX_3, LOCAL_IP_PREFIX_4, "127.0.0.1", "1.1.1.1", "8.8.8.8")
COMMENT_TIME_FORMAT = "%-d %b %Y %H:%M:%S.%f"  # See datetime strftime formats.

#Set comma separated value of snort alerts severity which will be blocked in Mikrotik. All severity values are ("1","2","3")
SEVERITY=("1","2") 
...
# Snort log file
SNORT_DATA_LOG_PATH="/var/log/snort"
FILEPATH = os.path.abspath(SNORT_DATA_LOG_PATH + "snort.alert.fast")
...
SAVE_LISTS_LOCATION = os.path.abspath("/var/lib/mikrosnort/savelists.json")

# Location for Mikrotik's uptime. (needed for re-adding lists after reboot)
UPTIME_BOOKMARK = os.path.abspath("/var/lib/mikrosnort/uptime.bookmark")

# Ignored rules file location - check ignore.conf for syntax.
IGNORE_LIST_LOCATION = os.path.abspath("/var/lib/mikrosnort/ignore.conf")

# Add all alerts from snort.alert.fast on start?
# Setting this to True will start reading snort.alert.fast from beginning
    ...
    if not ADD_ON_START:
        while True:
            try:
                ...
                print(f"[Mikrosnort] File: {fpath} not found. Retrying in 10 seconds..")
                sleep(10)
                continue
    ...
    while True:
        try:
            ...
            if "invalid user name or password" in str(e):
                print("[Mikrosnort] Invalid username or password.")
                sleep(10)
                continue
            ...
            print(f"[Mikrosnort] Socket timeout: {str(e)}.")
            sleep(30)
            continue

        except ConnectionRefusedError:
            print("[Mikrosnort] Connection refused. (api-ssl disabled in router?)")
            sleep(10)
            continue

        except OSError as e:
            if e.errno == 113:
                print("[Mikrosnort] No route to host. Retrying in 10 seconds..")
                sleep(10)
                continue

            if e.errno == 101:
                print("[Mikrosnort] Network is unreachable. Retrying in 10 seconds..")
                sleep(10)
                continue
...
def read_ignore_list(fpath):
    ...
    try:
        ...
        print(f"[Mikrosnort] File: {IGNORE_LIST_LOCATION} not found. Continuing..")
...
def main():
    ...
    while True:
        try:
            ...
            print(f"[Mikrosnort] (4) {str(e)}")
            connect_to_tik()
            continue

        except librouteros.exceptions.TrapError as e:
            print(f"[Mikrosnort] (8) librouteros.TrapError: {str(e)}")
            continue

        except KeyError as e:
            print(f"[Mikrosnort] (8) KeyError: {str(e)}")
            continue
